...
class fmri_cond_model(nn.Module):
    def __init__(self, model_dict, num_voxels, cond_dim=1280, global_pool=True):
        super().__init__()
...
def main():
    ...
    fmri_encoder = fmri_cond_model(model_dict, num_voxels=4656, cond_dim=768, global_pool=False)
    ...
    lora_layers, optimizer, train_dataloader, lr_scheduler, fmri_encoder = accelerator.prepare(
        lora_layers, optimizer, train_dataloader, lr_scheduler, fmri_encoder
    )
        ...
        if accelerator.is_main_process:
            if args.validation_prompt is not None and epoch % args.validation_epochs == 0:
                ...
                for _ in range(args.num_validation_images):
                    images.append(
                        pipeline(args.validation_prompt, num_inference_steps=30, generator=generator, guidance_scale = 1).images[0]
                    )
